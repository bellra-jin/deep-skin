...
| 검증 범위 | 볼·눈가·이마·턱 4부위 — 분류·회귀 30여 조건, 전부 5시드 · 실험 표 19개를 로그에서 재현 |
...
- 이마·턱까지 부위 확장(11,050 / 11,047장) — **사진으로 읽히는 지표와 아닌 지표를 갈라** 리포트 게재 기준 수립
...
담당 부위는 **볼(5·6) · 눈가(3·4) · 이마(1) · 턱(8)**, 예측 대상은 모공·색소침착·주름·처짐 등급과 장비 측정값입니다.
...
### 7.3 부위 확장 — 검증한 두 부위가 하필 어려운 쪽이었다

볼·눈가만으로는 위 수치가 높은지 낮은지 판단할 기준이 없습니다. 크롭 파이프라인을 이마(1)·턱(8)까지 넓혀 같은 프로토콜로 재봤습니다.

| 부위 · 타깃 | 등급 | macro-F1 | QWK | 폰 F1 |
|---|---:|---:|---:|---:|
| 턱 처짐 | 3 | **0.7723±0.0040** | 0.8095±0.0075 | 0.7201±0.0091 |
| 이마 색소 | 3 | 0.6609±0.0039 | 0.7232±0.0140 | 0.5993±0.0195 |
| 이마 주름 | 3 | 0.6489±0.0036 | 0.7331±0.0145 | 0.5966±0.0123 |
| 눈가 주름 | 3 | 0.6244±0.0026 | 0.7981±0.0057 | 0.6077±0.0135 |
| **볼 모공** | 3 | **0.5787±0.0028** | **0.4546±0.0033** | 0.4672±0.0165 |
...
**볼 모공이 전 부위 최하위이고, QWK 0.4546은 유일하게 0.5 미만입니다.** 등급 순서 구조가 가장 약한 과제를 먼저 붙잡고 있었던 셈입니다. 턱 처짐과 비교하면 macro-F1 차이가 0.19입니다.

이건 §8에서 볼 모공 등급이 장비 측정값과 갈라지는 지점을 찾은 것과 같은 이야기입니다 — **모델이 약한 게 아니라 그 라벨의 경계가 약합니다.**

병합 경계는 macro-F1으로 고르지 않았습니다(§8). 이마 색소에서 두 지표가 반대를 가리켰습니다 — 분포 균등 경계가 macro-F1은 0.6427로 낮은데 QWK는 0.7232 → 0.5854로 크게 떨어집니다. 등급 0을 따로 떼면 순서 정보가 깨지는 것으로 보여 **척도 위치 경계를 택했습니다.** 턱은 균등 경계가 세 지표 모두에서 나았고, 이마 주름은 두 방식이 같은 절단점으로 수렴했습니다.
...
### Phase 1. 미간·입술 부위 마무리

`crop.py`의 부위 테이블·품질 임계값·각도 집합은 미간(2)·입술(7)까지 이미 넣어 두었습니다. equipment 측정값이 없어 분류만 가능하므로 `--faceparts 2` 한 줄로 이어갈 수 있습니다. 완료하면 서비스 8부위 중 6부위가 같은 프로토콜로 검증됩니다.
