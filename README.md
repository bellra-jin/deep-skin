...
> 팀 프로젝트가 끝난 뒤에는 **AI-Hub 원본을 직접 받아 그 진단을 다시 검증했습니다** — 각도 필터의 오류를 찾아 학습 데이터를 48% 늘리고, GPU 없이 23개 조건을 비교해 백본 교체의 효과를 수치로 확인했습니다(§6.5~6.8).
...
AI-Hub 원천 이미지 + 라벨 JSON
 ...
 ③ 등급 분류 모델 학습          ← 담당
    ResNet-50 · 불균형 대응 · 3등급 전환 실험 · 한계 진단
        │
        ├─ (포크 이후 개인 작업) 각도 필터 검증 · 특징 캐싱 · 백본 비교 23건
        │
        ▼
 ④ AI 추론 서버 (팀 공동 산출물)
    부위 검출 + 멀티태스크 추론
...
작업 환경은 **CUDA 미지원 노트북(물리 4코어 CPU)** 입니다. 이 제약이 오히려 설계를 결정했습니다(§6.7).
...
### 6.7 GPU 없이 23개 실험을 돌리는 법 — 백본 동결 + 특징 캐싱
...
23개 실험 전체 기록은 `results/head_experiments.csv`에 있습니다. 여기서는 결론이 나온 네 가지만 정리합니다.
...
CORAL(서수 손실)의 거동도 이 해석을 뒷받침합니다. **ResNet-50 위에서 CORAL은 6등급 macro-F1을 0.378 → 0.356으로 떨어뜨렸지만, DINOv3 위에서는 0.400 → 0.429로 올립니다.** 서수 손실은 특징이 이미 등급 순서를 담고 있을 때만 그 구조를 살릴 수 있습니다.
...
**정리하면**, §6.4의 "백본의 한계"라는 진단은 **같은 조건에서 재현됐고**, 개선 방향도 세 가지로 구체화됐습니다 — 등급 병합(문제 정의), 백본 교체(표현력), 서수 손실(라벨 구조). 남은 격차는 여전히 기기 도메인 갭입니다.
...
results/head_experiments.csv       # (6.8) 실험 23건 누적 기록 — 조건과 지표가 한 줄씩
...
`train_head.py`가 모든 실행을 `results/head_experiments.csv`에 누적합니다(조건 · 지표 · 기기별 성능 · 소요 시간). 남은 것은 `train.py`(전체 미세조정 경로)에도 같은 기록을 붙이는 일입니다.
...
### Phase 4. 촬영 기기 도메인 갭 해소

§6.8에서 디카 학습 → 폰 검증 성능이 같은 조건 검증 대비 뚜렷하게 낮았습니다(모공 0.563 → 0.505). 백본 교체로 격차를 줄였지만 남아 있습니다. 디카 이미지에 JPEG 압축 · 다운스케일 · 블러를 적용한 증강 특징을 함께 캐싱해, **서비스 입력 분포에 맞춘 학습**을 시도할 수 있습니다. 캐싱 구조가 이미 있으므로 추출 1회로 검증 가능합니다.
