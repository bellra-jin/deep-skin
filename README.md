...
| 검증 범위 | 리포트가 보여주는 **6개 부위 전부**(facepart 1~8) — 분류·회귀 35여 조건, 전부 5시드 · 실험 표 20개를 로그에서 재현 |
...
- 이마·미간·입술·턱까지 부위 확장(부위당 1.1만 장) — **사진으로 읽히는 지표와 아닌 지표를 갈라** 리포트 게재 기준 수립
- 등급 경계를 장비 측정값으로 재설계 — 볼 모공 QWK **0.455 → 0.502**, 폰 F1 **0.467 → 0.587** (단, 눈가에서는 반대로 나빠져 **조건부 결론**으로 기록)
...
담당 부위는 **볼(5·6) · 눈가(3·4) · 이마(1) · 미간(2) · 입술(7) · 턱(8)**, 예측 대상은 모공·색소침착·주름·처짐·건조 등급과 장비 측정값입니다.
...
> 중앙 부위에 제외 각도가 없는 것도 같은 통계로 확인했습니다 — 볼은 각도에 따라 bbox 가로/세로 비가 **8.1배** 흔들리는데(0.171~1.391) 이마는 1.34배, 턱은 1.25배뿐입니다. 어느 각도에서도 정면을 향한 채 보입니다. 각도 7·8을 빼면 패드·폰 레코드가 정면 하나만 남아 **기기 다양성이 무너지는** 문제도 있습니다.
>
...
### 7.3 부위 확장 — 과제마다 난이도가 크게 다르다

볼·눈가만으로는 위 수치가 높은지 낮은지 판단할 기준이 없습니다. 크롭 파이프라인을 **이마(1)·미간(2)·입술(7)·턱(8)** 까지 넓혀 같은 프로토콜로 재봤습니다. 좌우 쌍(눈가 3·4, 볼 5·6)을 합치면 **리포트가 보여주는 6개 부위 전부**입니다.

| 부위 · 타깃 | 등급 | crop 중앙(px) | macro-F1 | QWK | 폰 F1 |
|---|---:|---:|---:|---:|---:|
| 턱 처짐 | 3 | 1,444 | **0.7723±0.0040** | 0.8095±0.0075 | 0.7201±0.0091 |
| 이마 색소 | 3 | 1,070 | 0.6609±0.0039 | 0.7232±0.0140 | 0.5993±0.0195 |
| 이마 주름 | 3 | 1,070 | 0.6489±0.0036 | 0.7331±0.0145 | 0.5966±0.0123 |
| 미간 주름 | 3 | 248 | 0.6413±0.0018 | 0.7364±0.0103 | 0.6030±0.0217 |
| 눈가 주름 | 3 | 273 | 0.6244±0.0026 | 0.7981±0.0057 | 0.6077±0.0135 |
| 볼 모공 | 3 | 631 | 0.5787±0.0028 | **0.4546±0.0033** | 0.4672±0.0165 |
| **입술 건조** | 3 | 675 | **0.4647±0.0054** | **0.2638±0.0205** | 0.4290±0.0192 |
| 턱 처짐 | 7 | — | 0.4315±0.0035 | 0.8207±0.0078 | 0.3875±0.0167 |
| 이마 주름 | 7 | — | 0.4539±0.0063 | 0.7900±0.0080 | 0.4008±0.0166 |
| 이마 색소 | 6 | — | 0.3796±0.0174 | 0.6690±0.0088 | 0.3184±0.0123 |

**3등급 기준으로 최상위와 최하위의 macro-F1 차이가 0.31입니다.** 볼 모공과 입술 건조만 QWK가 0.5 미만으로, 등급 순서 구조가 약합니다. 처음 붙잡은 두 부위가 하필 그 어려운 쪽이었습니다.

볼 모공은 §8에서 장비 측정값과 갈라지는 지점을 찾은 그 라벨입니다 — **모델이 약한 게 아니라 그 라벨의 경계가 약합니다.** 입술 건조도 등급 2가 61.3%이고 양 끝이 2%대로 극히 희소한데, **측정값이 없어 같은 방식으로 확인할 수 없습니다.**

병합 경계는 macro-F1으로 고르지 않았습니다(§8). 이마 색소에서 두 지표가 반대를 가리켰습니다 — 분포 균등 경계가 macro-F1은 0.6427로 낮은데 QWK는 0.7232 → 0.5854로 크게 떨어집니다. 등급 0을 따로 떼면 순서 정보가 깨지는 것으로 보여 **척도 위치 경계를 택했습니다.** 채택하지 않은 경계까지 로그에 남겨 **선택 근거 자체가 재현되게** 했습니다.

#### 크롭 해상도로는 설명되지 않는다

부위마다 크롭 크기가 크게 다릅니다 — 중앙값이 248px(미간)에서 1,444px(턱)까지 **5.8배**이고, 미간과 눈가는 절반가량이 DINOv3 입력(256px)보다 작아 **업샘플링해서 들어갑니다.**

그런데 성능과 붙지 않습니다. **가장 작은 미간(248px)이 중위권**이고 이마(1,070px)와 사실상 같으며, **크롭이 2.7배 큰 입술(675px)이 최하위**입니다. 순위 상관도 macro-F1과 +0.607, QWK와 +0.107로 지표에 따라 달라집니다.

n=7이고 부위마다 라벨 난이도·클래스 수·표본 수가 다르므로 **관계가 있다고도 없다고도 단정하지 않습니다.** 다만 해상도로 설명되지 않는 사례가 양쪽 끝에 다 있다는 것은 기록해 둡니다.

> 볼의 `crop_width`는 컬럼이 추가되기 전에 생성된 CSV라, `facepart_audit`의 bbox에 같은 여백 규칙을 적용해 계산한 값입니다. 나머지 부위는 크롭 시점의 실측값입니다.

### 7.4 분류 + 회귀 멀티태스크 — 사진으로 읽히는 지표와 아닌 지표

캐싱된 특징 위에 회귀 헤드를 얹어 **장비 측정값을 직접 예측**하도록 확장했습니다. 특징 재추출 없이 헤드와 손실 함수만 바뀝니다.

처음 세 타깃에서 **볼 수분만 유독 약했습니다.** 수분이라는 지표의 성질인지 볼이라는 부위의 문제인지 가르기 위해, 같은 수분·탄력을 이마·턱에서도 쟀습니다(세 부위 모두 결측 0%, 스케일도 거의 같아 직접 비교가 성립합니다).

| 타깃 · 부위 | ρ | R² | MAE(원 단위) | MAE/sd |
|---|---:|---:|---:|---:|
| 모공 개수 · 볼 | **0.7684±0.0026** | 0.509±0.035 | 206.7±8.5 | 0.44 |
| 거칠기 Ra · 눈가 | **0.7396±0.0023** | 0.531±0.008 | 3.09±0.02 | 0.56 |
| 수분 · 볼 | 0.5496±0.0018 | 0.268±0.005 | 7.75±0.04 | 0.76 |
| 탄력 R2 · 볼 | 0.5284±0.0024 | 0.298±0.016 | 0.064±0.001 | 0.66 |
| 수분 · 턱 | 0.5167±0.0086 | 0.256±0.008 | 7.99±0.06 | 0.74 |
| 탄력 R7 · 이마 | 0.4733±0.0074 | 0.168±0.029 | 0.043±0.001 | 0.64 |
| 탄력 R2 · 턱 | 0.4006±0.0054 | 0.145±0.003 | 0.087±0.000 | 0.83 |
| 수분 · 이마 | 0.3828±0.0057 | 0.095±0.022 | 7.20±0.11 | 0.74 |
| 탄력 R2 · 이마 | 0.3759±0.0073 | 0.103±0.009 | 0.072±0.000 | 0.66 |
| 탄력 R7 · 턱 | 0.3387±0.0096 | 0.117±0.006 | 0.055±0.000 | 0.81 |

**부위 문제가 아니었습니다.** 수분은 세 부위 전부 ρ 0.38~0.55이고 이마는 오히려 볼보다 낮습니다. 탄력도 같은 대역입니다.

**지표가 두 무리로 갈립니다.**

```text
사진에 형태로 드러나는 것       모공 개수 · 표면 거칠기      ρ 0.74 ~ 0.77
피부 안쪽 물성                  수분 · 탄력                 ρ 0.34 ~ 0.55
                                                            ↑ 0.55~0.74 사이에 아무 지표도 없다
```

후자는 장비가 피부에 접촉해 재는 값이라 사진에 직접 나타나지 않고, 모델이 간접 단서로 추정하는 것으로 **보입니다 — 이건 측정하지 않은 해석입니다.** 탄력은 14종 중 대표 2종만 쟀습니다.

회귀를 붙여도 분류는 나빠지지 않습니다.
...
### 7.5 등급 경계를 측정값으로 다시 끊었다 — 볼에서만 이겼다

§8에서 "볼 모공 등급이 정보를 잃고 있다"고 해석했고, §7.3에서 그 볼 모공이 전 부위 최하위(QWK 0.4546)로 나왔습니다. 두 사실이 같은 이야기라면, **경계를 측정값으로 다시 끊으면 더 학습 가능해져야** 합니다. 같은 이미지·같은 특징·같은 헤드로 **라벨의 경계만** 바꿔 확인했습니다.

**클래스 비율을 고정하는 것이 설계의 전부입니다.** §8에서 "등급 병합이 듣는 이유는 클래스 균형"이라고 결론냈으므로, 측정값을 균등하게 끊어 macro-F1이 오르는 것은 당연하고 새 정보가 아닙니다. 그래서 측정값 절단점을 **전문가 3등급과 같은 클래스 비율이 되도록**(20.65 : 60.69 : 18.66%) 잡았습니다. 절단점은 train 분포에서만 산출했습니다.

두 라벨은 실제로 다릅니다 — 일치율 63.6%, Cohen's kappa 0.270. **전문가가 "주의"로 본 407건 중 274건이 측정값 기준으로는 중간**입니다.

| 볼 모공 (5시드) | macro-F1 | QWK | 폰 F1 | 승 |
|---|---:|---:|---:|:--:|
| 전문가 3등급 | 0.5787±0.0028 | 0.4546±0.0033 | 0.4672±0.0165 | — |
| **측정값 3등급** | **0.6167±0.0043** | **0.5023±0.0118** | **0.5868±0.0165** | 5/5 |

| 눈가 주름 (5시드) | macro-F1 | QWK | 폰 F1 | 승 |
|---|---:|---:|---:|:--:|
| **전문가 3등급** | **0.6244±0.0026** | **0.7981±0.0057** | **0.6077±0.0135** | — |
| 측정값 3등급 | 0.5425±0.0034 | 0.5926±0.0097 | 0.5214±0.0195 | 0/5 |

**볼에서는 실증되고 눈가에서는 정반대입니다.**

설명은 기준선에 있습니다. 볼 모공의 원래 QWK는 0.4546으로 전 부위 최하위였고, 눈가 주름은 0.7981로 최상위권이었습니다. **측정값 경계는 전문가 경계가 약했던 곳에서만 이깁니다.** 이미 잘 정렬된 라벨을 분위수로 갈아엎으면 순서 구조가 무너집니다(QWK 0.798 → 0.593).

**눈가를 돌리지 않았다면 "측정값 경계가 더 학습 가능하다"를 일반 명제로 쓸 뻔했습니다.** 한 부위에서 나온 결과는 부위 특이성인지 일반 현상인지 구분되지 않습니다.

연속 타깃으로 배우는 편이 쉬운지도 부위마다 갈립니다 — 회귀 예측을 같은 절단점으로 이산화하면 볼은 −0.010, 눈가는 **+0.058**입니다. 다만 **회귀→이산화는 기기 도메인 이동에 훨씬 취약합니다**: 폰 F1이 0.23~0.26으로 무너지고(직접 분류 0.52~0.59), 눈가의 디카−폰 격차는 0.008 → 0.106으로 14배 벌어집니다. 원인은 이 실험이 답할 수 없어 관찰까지만 적습니다.

> **한계** — 이 실험은 "이미지 특징으로 어느 경계가 더 학습 가능한가"만 물었습니다. **어느 등급이 사용자에게 유용한지, 장비가 전문가보다 정확한지는 답하지 않습니다.** 전문가는 시각적 심각도를, 장비는 개수 하나를 봅니다.
> 또 절단점을 train 비율로 맞춘 탓에 val에서는 비율이 14.0 : 72.0 : 13.9%로 전문가(16.4 : 62.3 : 21.3%)와 어긋납니다. **볼의 +0.038 중 일부가 이 차이에서 왔을 가능성을 배제하지 못합니다.**
> 성능이 올랐다고 라벨을 바꾸자는 결론으로 가지 않았습니다 — 등급 정의를 바꾸면 §9.2의 severity 매핑, 기존 저장 결과, 사용자가 본 과거 리포트가 전부 영향을 받습니다.
...
그래서 부위를 확장할 때 병합 경계를 macro-F1으로 고르지 않았습니다. 이마 색소에서 실제로 두 지표가 반대를 가리켰고, QWK를 근거로 macro-F1이 더 낮은 경계를 택했습니다(§7.3).

**이미지에는 등급이 담지 못한 정보가 있습니다.** 회귀 헤드로 측정값을 직접 예측하면 Spearman 0.768이 나옵니다(§7.4). 등급 라벨과 측정값의 상관은 0.444입니다. 이미지에는 측정값을 상당히 설명하는 정보가 들어 있는데 **등급 라벨이 그 정보를 많이 잃고 있다**는 뜻입니다.

> 모델의 0.768과 전문가 등급의 0.444를 나란히 놓고 "모델이 전문가보다 정확하다"고 읽으면 안 됩니다. 모델은 그 측정값을 맞히도록 학습됐고 전문가는 애초에 그럴 목적이 아니었습니다.

여기까지는 **해석**입니다. 측정값으로 등급을 다시 끊어 이 해석을 §7.5에서 실증했고, **조건부로만 참이었습니다.**
...
검출 실패를 추정값으로 표시하는 것과, **읽히지도 않는 지표를 예측값으로 내보내는 것**은 같은 문제입니다. §7.4의 결과로 지표별 판정 기준을 세웠습니다.

> **ρ ≥ 0.70 이고 MAE/sd ≤ 0.60 이면 숫자로 싣는다.**
>
> ρ 0.70은 두 무리 사이의 빈 구간입니다 — 0.55와 0.74 사이에 아무 지표도 없습니다.
> MAE/sd 0.60은 예측 오차가 개인차의 60% 미만이라는 뜻으로, 그보다 크면 "이 사람은 평균보다 수분이 높다"조차 말하기 어렵습니다.

| 지표 | 판정 | 근거 |
|---|---|---|
| 모공 개수 (볼) | **숫자로 싣는다** | ρ 0.768 · MAE/sd 0.44 |
| 거칠기 Ra (눈가) | **숫자로 싣는다** | ρ 0.740 · MAE/sd 0.56 |
| 수분 (볼·이마·턱) | 내지 않는다 | ρ 0.38~0.55 · MAE/sd 0.74~0.76 |
| 탄력 (볼·이마·턱) | 내지 않는다 | ρ 0.34~0.53 · MAE/sd 0.64~0.83 |
| 이마 색소·주름, 턱 처짐 | 등급으로만 | 회귀 타깃 없음 · 3등급 macro-F1 0.65~0.77 |

수분·탄력은 **원본 라벨에 100% 존재하지만 사진으로는 읽히지 않습니다.** §9.1에서 `chin_moisture`를 응답에서 뺀 판단이, 이제 "학습 대상이 아니라서"가 아니라 **"읽히지 않아서"** 라는 근거를 갖게 됐습니다.
...
출력이 README와 다르면 둘 중 하나가 틀린 것입니다. 현재 표 **17개**가 로그에서 재현됩니다.

이 장치에도 한 번 구멍이 있었습니다. 회귀 실험을 별도 CSV로 분리해 두고 **그 CSV를 소스로 등록하지 않아서**, `--check`가 통과하는 동안 회귀 표들은 아무것도 검사받지 않고 있었습니다. **초록불이 켜져 있는데 그 표는 아무도 보고 있지 않았던 것**입니다.

같은 모양의 실패를 이 저장소에서 세 번 봤습니다 — 검증셋에 표본이 0건인 등급쌍에서 그럴듯한 분리도를 낸 프록시, 확률이 몰려 구간이 비었는데도 작은 값을 낸 등폭 ECE, 그리고 이 `--check`입니다. **지표와 검증 장치는 죽지 않고 조용히 아무것도 재지 않을 수 있습니다.** 그래서 "통과했다"보다 **"무엇을 덮고 있는가"** 를 먼저 확인하게 됐습니다(§14.3의 커버리지 테스트도 같은 이유입니다).
...
**결론의 범위는 실험이 정합니다.** 측정값으로 등급을 다시 끊었더니 볼에서 macro-F1과 QWK가 함께 올랐고, 그대로 "측정값 경계가 더 학습 가능하다"고 쓸 참이었습니다. 눈가에서 같은 방식을 돌려 보니 **정반대로 크게 나빠졌습니다**(§7.5). 한 부위에서 나온 결과는 부위 특이성인지 일반 현상인지 구분되지 않는데, **문장으로 쓸 때는 그 구분이 사라집니다.** 대조군을 하나 더 두는 30분이 잘못된 일반 명제 하나를 막았습니다.

---

## 16. 고도화 방향

### Phase 1. 입술 건조 등급 — 가장 약한 라벨

§7.3에서 입술 건조가 **QWK 0.2638로 전 과제 최하위**로 드러났습니다. 등급 2가 61.3%, 양 끝이 2%대로 볼 모공보다 쏠림이 심합니다. 그런데 **입술에는 equipment 측정값이 없어** §7.5에서 볼 모공에 쓴 방법(측정값 분위수로 경계 재설계)을 그대로 적용할 수 없습니다. 측정값 없이 라벨 품질을 재는 방법이 필요하며, §14.5에서 시도한 모델 혼동도 프록시는 검증을 통과하지 못했습니다.

### Phase 2. 등급 재설계가 어디까지 통하는가

§7.5에서 측정값 기반 경계가 **볼에서는 이기고 눈가에서는 졌습니다.** 기준선 QWK가 낮은 부위에서만 이긴다는 설명이 붙었지만, **두 부위로 세운 가설**입니다. 이마·턱까지 넓히면 "기준선 QWK가 어느 선 아래일 때 재설계가 유효한가"를 숫자로 말할 수 있습니다. 특징도 라벨도 이미 있어 head 학습만 돌리면 됩니다.

한 가지 더 — 회귀→이산화는 정확도가 오른 부위에서도 **폰 F1이 0.23까지 무너졌습니다**(직접 분류 0.52~0.59). 회귀 헤드가 기기 도메인 이동에 왜 그렇게 취약한지는 §7.2의 도메인 적응을 회귀 쪽에 붙여 봐야 답이 나옵니다.

### Phase 3. 피부 안쪽 물성을 읽는 다른 입력

수분·탄력은 세 부위 모두 ρ 0.34~0.55로, 일반 사진으로는 읽히지 않는다는 결론이 났습니다(§7.4). 이 지표를 서비스에 실으려면 **모델이 아니라 입력을 바꿔야** 합니다 — 편광·근적외 촬영처럼 표면 아래를 보는 방식이 필요하며, 현재 데이터셋으로는 검증할 수 없습니다.

### Phase 4. 라벨 없는 신규 기기 대응
