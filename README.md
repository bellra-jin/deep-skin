...
| 검증 범위 | 볼·눈가·이마·턱 4부위 — 분류 8조건 · 회귀 10조건, 전부 5시드 |
...
§7.3에서 볼 모공이 전 부위 최하위(QWK 0.4546, 유일하게 0.5 미만)로 드러났고, §8에서 그 등급이 등급 2 이상부터 장비 측정값과 갈라지는 것을 확인했습니다. **모델이 아니라 라벨 정의를 다시 볼 자리**입니다 — 측정값 분위수로 등급을 다시 끊으면 전문가 등급보다 학습 가능한 경계가 나올 수 있습니다.
